...
profile actually exist before installing the cron job (`manifests/schedule.pp:74-99`).
...
- **`openscap` (`manifests/init.pp:16-35`)** — Public entry class. Calls
  `simplib::assert_metadata($module_name)` (`init.pp:22`). Parameters:
  ...
  - `$ssg_ensure` (`String`) — ensure for `scap-security-guide`; same
    `simp_options::package_ensure` default (`init.pp:19`).
  - Declares `package { 'openscap-utils' }` and `package { 'scap-security-guide' }`
    (`init.pp:29-34`).

- **`openscap::schedule` (`manifests/schedule.pp:52-131`)** — Public class that
  sets up the recurring scan. `include`s `openscap` (`schedule.pp:68`). Key
  parameters:
  - `$scap_profile` (`Openscap::Profile`, **required, no default**) — the SSG
    profile id (`schedule.pp:53`).
  - `$ssg_data_stream` (`Pattern[/^.+\.xml$/]`, **required, no default**) — the
    data stream XML filename under `$ssg_base_dir` (`schedule.pp:54`).
  ...
  - `$ssg_base_dir` (`Stdlib::Absolutepath`, default
    `/usr/share/xml/scap/ssg/content`) (`schedule.pp:56`).
  - `$fetch_remote_resources` (`Boolean`, default `false`) (`schedule.pp:57`).
  - `$scap_tailoring_file` (`Optional[Stdlib::Absolutepath]`, default `undef`)
    (`schedule.pp:58`).
  - `$logdir` (`Stdlib::Absolutepath`, default `/var/log/openscap`) (`schedule.pp:59`).
  ...
  - `$minute`/`$hour`/`$monthday`/`$month`/`$weekday` — cron schedule, typed with
    `Simplib::Cron::*`, defaulting to `30`/`1`/`*`/`*`/`1` (`schedule.pp:61-65`).
  - `$force` (`Boolean`, default `false`) — bypass the fact-based validation and
    install the schedule anyway (`schedule.pp:66`).

  Control flow (`schedule.pp:70-130`):
  - If `$force`, `$_set_schedule = true` unconditionally (`schedule.pp:70-72`).
  ...
  - When `$_set_schedule`, it declares `file { $logdir }` (`ensure => directory`,
    ...
    `templates/oscap_command.erb` (`schedule.pp:102-117`).
  - If `$logrotate`, `include`s `logrotate` and declares a
    `logrotate::rule { 'openscap' }` rotating `${logdir}/*.xml` daily, keeping 3
    (`schedule.pp:119-129`).

- **`Openscap::Profile` (`types/profile.pp:2`)** — `Pattern[/xccdf_[^_]+_profile_.+/]`;
...
- **`oscap` fact (`lib/facter/oscap.rb`)** — `confine`d to hosts where `oscap` is
  ...
  (`oscap.rb:35-36`). Returns a hash of `path`, `version`, `supported_specifications`,
  and a nested `profiles` map keyed by directory → data stream → profile id
  (`oscap.rb:34-95`). It parses data-stream XML with a regex scan rather than a
...
- **The schedule is silently skipped when `oscap` is not installed.** Without
  ...
  `warning` notify is emitted (`schedule.pp:93-99`). The fact itself is `confine`d
  to hosts that already have `oscap` on the PATH *and* SSG data streams present
  (`oscap.rb:35-36`), so on a first run before the packages are installed the fact
  will not exist. Use `$force` to install the schedule regardless
  (`schedule.pp:44-48,70-72`).
- **Profile/data-stream validation happens at catalog-compile time** against the
  ...
  if the requested data stream or profile is missing (`schedule.pp:77-88`). This
...
- **`$scap_profile` is validated by a type pattern**, not a free string:
  `Openscap::Profile = Pattern[/xccdf_[^_]+_profile_.+/]` (`types/profile.pp:2`).
- **`$ssg_data_stream` must end in `.xml`** (`Pattern[/^.+\.xml$/]`,
  `schedule.pp:54`); the fact keys data streams by their `.xml`-stripped basename,
...
- **The cron log directory is mode `0600`** (`schedule.pp:104-106`) and results are
...
- **`logrotate` is a hard `metadata.json` dependency** but is only `include`d when
  `$logrotate` is true (`schedule.pp:119-120`); the `simp_options::logrotate`
  lookup defaults it to `false` (`schedule.pp:60`).
- **The ERB emits `--fetch-remote_resources`** (with an underscore) rather than
  ...
  version before relying on remote-resource fetching (`templates/oscap_command.erb:14`).
...
|------|-----|-----------------|
| `manifests/init.pp:18` | `simp_options::package_ensure` | `'installed'` |
| `manifests/init.pp:19` | `simp_options::package_ensure` | `'installed'` |
| `manifests/schedule.pp:60` | `simp_options::logrotate` | `false` |
